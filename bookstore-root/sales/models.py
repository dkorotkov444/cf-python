...
class Sale(models.Model):
    ...
    price = models.DecimalField(max_digits=10, decimal_places=2)        # Total sale price
    date_created = models.DateTimeField(auto_now_add=True)             # Date of sale - will be automatically set to the current date

    def __str__(self):
        return f"ID:{self.id}, book: {self.book.title}, quantity: {self.quantity}, price: $ {self.price}"
